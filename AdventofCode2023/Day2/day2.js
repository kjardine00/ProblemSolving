...
getAllPossible = (games) => {
  ...
  games.forEach((game) => {
    ...
    game.handfuls.forEach((handful) => {
      ...
      if (
        handful.red > numOfRed || handful.green > numOfGreen || handful.blue > numOfBlue
        ) {
        isPossible = false;
      }
    });
...
answer = (input) => { 
    ...
    const possibleGames = getAllPossible(gamesAsObjects);
    const answer = possibleGames.reduce(sum, 0);
